        ...
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-3 print:p-0 print:bg-white" onMouseDown={()=>setShowQuotePreview(false)}>
          <div className="quote-print w-full max-w-5xl max-h-[94vh] overflow-y-auto bg-white shadow-2xl p-4 text-black" onMouseDown={e=>e.stopPropagation()}>
            ...
            <div className="quote-sheet">
              ...
              <div className="flex justify-end"><table className="w-[30%]"><tbody><tr><td className="w-[43%] text-right whitespace-nowrap">未稅金額</td><td className="text-right whitespace-nowrap">NT${Number(previewQuote.quote.SubtotalAmount||0).toLocaleString()}</td></tr><tr><td className="text-right">含稅金額</td><td className="text-right whitespace-nowrap">NT${Math.round(Number(previewQuote.quote.SubtotalAmount||0)*1.05).toLocaleString()}</td></tr><tr><td className="text-right font-bold">優惠金額（含稅）</td><td className="text-right whitespace-nowrap font-bold">NT${Number(previewQuote.quote.DiscountAmount ?? previewQuote.quote.FinalAmount ?? previewQuote.quote.TotalAmount ?? 0).toLocaleString()}</td></tr></tbody></table></div>
