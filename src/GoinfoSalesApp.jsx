...
export default function App() {
  ...
  const quoteFormalDetails = (detail) => {
    ...
    const lines = detail.items.map(item => {
      ...
      const addUserTaxIncluded = maintenance ? Math.round(Number(maintenance.AdditionalUserPrice || 0)) : null;
      return { ...item, SystemCode: item.SystemCode || system.SystemCode, SystemName: item.SystemName || system.SystemName, Note: system.Note || '', maintenanceTaxIncluded, addUserTaxIncluded };
    });
  ...
  return (
    <div className="h-screen bg-gray-100 flex flex-col md:flex-row font-sans overflow-hidden">
      ...
      {showQuotePreview && previewQuote && <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4" onMouseDown={()=>setShowQuotePreview(false)}><div className="w-full max-w-4xl max-h-[90vh] overflow-y-auto bg-white rounded-xl shadow-2xl p-7" onMouseDown={e=>e.stopPropagation()}><div className="flex justify-between items-start border-b pb-4 mb-5"><div><h2 className="text-2xl font-bold">高益營建軟體－系統報價單</h2><div className="text-sm text-gray-500 mt-1">報價單號：{previewQuote.quote.QuotationNo}　日期：{formatDateForInput(previewQuote.quote.QuoteDate)}</div></div><button onClick={()=>setShowQuotePreview(false)} className="text-2xl text-gray-400">×</button></div><div className="grid grid-cols-2 gap-3 mb-5 text-sm"><div><span className="text-gray-500">客戶名稱：</span>{previewQuote.quote.CustomerName || previewQuote.quote.CustomerCode || "－"}</div><div><span className="text-gray-500">狀態：</span>{quoteStatusLabel(previewQuote.quote.Status)}</div></div><table className="w-full text-sm border"><thead className="bg-gray-100"><tr><th className="p-2 text-left">系統名稱</th><th className="p-2 text-right">購買人數</th><th className="p-2 text-right">未稅牌價</th><th className="p-2 text-right">折數（%）</th><th className="p-2 text-right">優惠金額（含稅）</th><th className="p-2 text-right">最終優惠（含稅）</th></tr></thead><tbody>{previewQuote.items.map(item=><tr key={item.QuotationItemId} className="border-t"><td className="p-2">{item.SystemCode && item.SystemName ? `${item.SystemCode}－${item.SystemName}` : (item.SystemName || item.SystemCode || "－")}</td><td className="p-2 text-right">{item.UserCount}</td><td className="p-2 text-right">${Number(item.LineAmount||0).toLocaleString()}</td><td className="p-2 text-right">{item.Discount ?? 100}</td><td className="p-2 text-right">${Number(item.DiscountAmount||0).toLocaleString()}</td><td className="p-2 text-right">{item.FinalAmount == null ? '－' : `$${Number(item.FinalAmount).toLocaleString()}`}</td></tr>)}</tbody></table>{previewQuote.isNewPurchase && <div className="mt-7 text-left border-t pt-5 space-y-5"><div><h3 className="font-bold text-lg mb-3">系統說明</h3>{previewQuote.items.map(item=><div key={`note-${item.QuotationItemId || item.SystemId}`} className="mb-4"><div className="font-semibold">{item.SystemCode && item.SystemName ? `${item.SystemCode}－${item.SystemName}` : (item.SystemName || item.SystemCode)}</div><div className="whitespace-pre-wrap text-sm leading-6 text-gray-700">{item.Note || '尚未設定系統內容說明'}</div></div>)}</div><div><h3 className="font-bold text-lg mb-2">維護說明</h3><p className="text-sm leading-6">軟體系統自簽約日起十三個月內免費提供教育訓練及維護修復。第二年起，本報價系統年度維護費如下（含稅）：</p><ul className="text-sm leading-6 list-disc pl-5">{previewQuote.items.map(item=><li key={`maint-${item.QuotationItemId || item.SystemId}`}>{item.SystemCode && item.SystemName ? `${item.SystemCode}－${item.SystemName}` : (item.SystemName || item.SystemCode)}，{item.UserCount} 人版：{item.maintenanceTaxIncluded == null ? '尚未設定維護價格規則' : `NT$${item.maintenanceTaxIncluded.toLocaleString()}`}</li>)}</ul><div className="font-semibold text-right">年度維護費合計（含稅）：NT${previewQuote.maintenanceTotal.toLocaleString()}</div></div><div><h3 className="font-bold text-lg mb-2">增加授權說明</h3><p className="text-sm leading-6">日後若新增授權人數，依本報價折數計算；各系統每增加一位使用者的優惠價格如下（含稅）：</p><ul className="text-sm leading-6 list-disc pl-5">{previewQuote.items.map(item=><li key={`add-${item.QuotationItemId || item.SystemId}`}>{item.SystemCode && item.SystemName ? `${item.SystemCode}－${item.SystemName}` : (item.SystemName || item.SystemCode)}：{item.addUserTaxIncluded == null ? '尚未設定維護價格規則' : `NT$${item.addUserTaxIncluded.toLocaleString()}／人`}</li>)}</ul></div></div>}<div className="mt-5 text-right space-y-1"><div>未稅小計：${Number(previewQuote.quote.SubtotalAmount || 0).toLocaleString()}</div><div>含稅小計：${Math.round(Number(previewQuote.quote.SubtotalAmount || 0) * 1.05).toLocaleString()}</div><div className="text-xl font-bold text-blue-700">優惠金額（含稅）：${Number(previewQuote.quote.DiscountAmount ?? previewQuote.quote.FinalAmount ?? previewQuote.quote.TotalAmount ?? 0).toLocaleString()}</div></div><div className="mt-6 flex justify-end gap-3"><button onClick={()=>window.print()} className="px-4 py-2 bg-blue-600 text-white rounded-lg">列印</button><button onClick={()=>editQuote(previewQuote.quote)} className="px-4 py-2 border border-amber-500 text-amber-600 rounded-lg">帶入修改</button><button onClick={()=>setShowQuotePreview(false)} className="px-4 py-2 border rounded-lg">關閉</button></div></div></div>}
