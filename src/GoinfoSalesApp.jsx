...
export default function App() {
  ...
  const quoteFormalDetails = (detail) => {
    ...
    return { ...detail, isNewPurchase, items: lines, maintenanceTotal: lines.reduce((sum, x) => sum + Number(x.maintenanceTaxIncluded || 0), 0) };
  };
  ...
  const renderQuotationForm = (title, defaultItemType, actionType) => (
    ...
    <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
      ...
      {quoteItems.length>0 && <div className="border-t pt-4 text-right space-y-1"><div className="text-gray-500">原始牌價（未稅）：${quoteSummary.listAmount.toLocaleString()}</div><div className="text-gray-500">原始牌價（含稅）：${quoteSummary.taxIncludedListAmount.toLocaleString()}</div><div>優惠總計（未稅）：${quoteSummary.taxExcludedAmount.toLocaleString()}</div><div>營業稅（5%）：${quoteSummary.taxAmount.toLocaleString()}</div><div className="text-xl font-bold">優惠總計（含稅）：${quoteSummary.taxIncludedAmount.toLocaleString()}</div><div className="text-orange-600">最終議價（含稅）：-${(quoteSummary.discountAmount - quoteSummary.taxIncludedAmount).toLocaleString()}</div><button onClick={()=>handleSubmit(actionType)} className="mt-4 bg-blue-600 text-white px-6 py-2 rounded shadow">產生並存檔</button></div>}
    </div>
  ...
  return (
    <div className="h-screen bg-gray-100 flex flex-col md:flex-row font-sans overflow-hidden">
      ...
      {showQuotePreview && previewQuote && <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-3 print:p-0 print:bg-white" onMouseDown={()=>setShowQuotePreview(false)}><div className="quote-print w-full max-w-5xl max-h-[94vh] overflow-y-auto bg-white shadow-2xl p-4 text-black" onMouseDown={e=>e.stopPropagation()}><style>{`@media print { body * { visibility:hidden; } .quote-print, .quote-print * { visibility:visible; } .quote-print { position:absolute; left:0; top:0; width:100%; max-width:none; max-height:none; overflow:visible; padding:7mm; box-shadow:none; } .no-print { display:none !important; } @page { size:A4 portrait; margin:7mm; } } .quote-sheet { border:1px solid #111; padding:7mm; font-family:Arial, 'Microsoft JhengHei', sans-serif; font-size:11px; line-height:1.35; } .quote-sheet table { border-collapse:collapse; width:100%; } .quote-sheet th,.quote-sheet td { border:1px solid #222; padding:4px; vertical-align:top; } .quote-sheet .no-border td { border:0; padding:1px; }`}</style><div className="quote-sheet"><button onClick={()=>setShowQuotePreview(false)} className="no-print float-right text-2xl text-gray-400">×</button><header className="text-center border-b-2 border-black pb-2 mb-2"><div className="text-xl font-bold tracking-wide">高益電腦股份有限公司 Goinfo Auto Co., Ltd.</div><div className="text-[10px] text-left mt-1 leading-4">總公司台北　地址：台北市復興北路333號9樓之3　　電話：02-2713-7188　傳真：02-2713-4563<br/>總公司台中　地址：台中市文心路3段241號16樓之9　電話：04-2298-1378　傳真：04-2298-1328<br/>總公司高雄　地址：高雄市左營區大順一路93號5樓之5　電話：07-558-0520　傳真：07-558-0536</div><h1 className="text-xl font-bold mt-1">軟體買賣報價單</h1><div className="font-semibold">QUOTATION</div></header><table className="mb-2"><tbody><tr><td className="w-1/2">客戶名稱：{previewQuote.quote.CustomerName || previewQuote.quote.CustomerCode || '－'}</td><td>報價日期：{formatDateForInput(previewQuote.quote.QuoteDate)}</td></tr><tr><td>報價單號：{previewQuote.quote.QuotationNo}</td><td>報價類型：{quoteStatusLabel(previewQuote.quote.Status)}</td></tr></tbody></table><table><thead><tr className="text-center bg-gray-100"><th>品名</th><th>摘要</th><th>定價（未稅）</th><th>數量</th><th>小計（未稅）</th><th>備註</th></tr></thead><tbody>{previewQuote.items.map(item=><tr key={item.QuotationItemId || item.SystemId}><td>{item.SystemCode && item.SystemName ? `${item.SystemCode}－${item.SystemName}` : (item.SystemName || item.SystemCode || '－')}</td><td>{item.UserCount} 人版</td><td className="text-right">NT${Number(item.LineAmount || 0).toLocaleString()}</td><td className="text-center">1</td><td className="text-right">NT${Number(item.LineAmount || 0).toLocaleString()}</td><td>折數 {item.Discount ?? 100}%<br/>優惠含稅 NT${Number(item.DiscountAmount || 0).toLocaleString()}</td></tr>)}</tbody></table><div className="flex justify-end mt-2"><table className="w-60"><tbody><tr><td>未稅金額</td><td className="text-right">NT${Number(previewQuote.quote.SubtotalAmount || 0).toLocaleString()}</td></tr><tr><td>含稅金額</td><td className="text-right">NT${Math.round(Number(previewQuote.quote.SubtotalAmount || 0)*1.05).toLocaleString()}</td></tr><tr><td>優惠金額（含稅）</td><td className="text-right font-bold">NT${Number(previewQuote.quote.DiscountAmount ?? previewQuote.quote.FinalAmount ?? previewQuote.quote.TotalAmount ?? 0).toLocaleString()}</td></tr></tbody></table></div>{previewQuote.isNewPurchase && <section className="mt-3 text-[10px]"><h2 className="font-bold text-sm border-b border-black">系統說明</h2>{previewQuote.items.map(item=><div key={`note-${item.QuotationItemId || item.SystemId}`} className="mt-1"><b>{item.SystemCode && item.SystemName ? `${item.SystemCode}－${item.SystemName}` : (item.SystemName || item.SystemCode)}：</b><span className="whitespace-pre-wrap">{item.Note || '尚未設定系統內容說明'}</span></div>)}<h2 className="font-bold text-sm border-b border-black mt-3">維護說明</h2><ol className="list-decimal pl-5 space-y-1"><li>軟體系統自簽約日起壹年內免費提供教育訓練及維護修復。第二年起{previewQuote.items.map((item,i)=><span key={`m-${item.SystemId}`}>{i ? '、' : ''}{item.SystemCode}（網路 {item.UserCount} 人版）</span>)}維護費每年合計為新台幣<b>NT${previewQuote.maintenanceTotal.toLocaleString()}</b>元整（含稅）。日後若新增授權人數，各系統每增加一使用者維護費如下：{previewQuote.items.map((item,i)=><span key={`am-${item.SystemId}`}>{i ? '；' : ''}{item.SystemCode} NT${Number(item.addUserMaintenanceTaxIncluded || 0).toLocaleString()}（含稅）</span>}。</li><li>網路版同時作業人數，各系統每增加一人優惠金額如下（含稅）：{previewQuote.items.map((item,i)=><span key={`al-${item.SystemId}`}>{i ? '；' : ''}{item.SystemCode} NT${Number(item.licenseAddUserTaxIncluded || 0).toLocaleString()}</span>}。</li></ol></section>}<section className="mt-3 text-[10px]"><div>說明：1. 本報價單以上金額含稅，有效期至 {formatDateForInput(previewQuote.quote.ValidUntil) || '－'}。　2. 安裝完成後30日內，100%（30日到期票）。</div><table className="mt-2"><tbody><tr className="h-24"><td className="w-[34%] text-center align-top pt-2">客戶確認簽章</td><td className="w-[30%] text-center align-middle"><div className="inline-flex h-20 w-20 rounded-full border-4 border-gray-400 items-center justify-center text-center text-gray-500 font-bold rotate-[-12deg]">高益電腦<br/>報價專用章</div></td><td className="w-[36%] p-0"><table className="h-full"><tbody><tr><td>承辦人</td><td>產品規劃部副理　鐘廷睿<br/>電話：(04)2298-1378#20</td></tr><tr><td>傳真</td><td>(04)2298-1328</td></tr><tr><td>承辦人簽名</td><td></td></tr></tbody></table></td></tr></tbody></table></section></div><div className="no-print mt-4 flex justify-end gap-3"><button onClick={()=>window.print()} className="px-4 py-2 bg-blue-600 text-white rounded-lg">列印</button><button onClick={()=>editQuote(previewQuote.quote)} className="px-4 py-2 border border-amber-500 text-amber-600 rounded-lg">帶入修改</button><button onClick={()=>setShowQuotePreview(false)} className="px-4 py-2 border rounded-lg">關閉</button></div></div></div>}
