...
export default function App() {
  ...
  const quoteValidDate = (quote) => { const d = new Date(quote.QuoteDate); d.setDate(d.getDate()+30); return formatRocDate(d); };
          ...
          <div className="quote-print w-full max-w-5xl max-h-[94vh] overflow-y-auto bg-white shadow-2xl p-4 text-black" onMouseDown={e=>e.stopPropagation()}>
            ...
            <div className="quote-sheet">
              ...
              <header className="border-b-2 border-black pb-2 mb-2">
                ...
                <table className="noborder compact mt-1"><tbody><tr><td className="w-16">總公司台北</td><td>地址：台北市復興北路333號9樓之3</td><td>電話：02-2713-7188</td><td>傳真：02-2713-4563</td></tr><tr><td>總公司台中</td><td>地址：台中市文心路3段241號16樓之9</td><td>電話：04-2298-1378</td><td>傳真：04-2298-1328</td></tr><tr><td>總公司高雄</td><td>地址：高雄市左營區大順一路93號5樓之5</td><td>電話：07-558-0520</td><td>傳真：07-558-0536</td></tr></tbody></table>
              ...
              <table><colgroup><col className="w-[48%]"/><col className="w-[15%]"/><col className="w-[12%]"/><col className="w-[8%]"/><col className="w-[17%]"/></colgroup><thead><tr className="text-center bg-gray-100"><th>品名</th><th>定價（未稅）</th><th>數量</th><th>小計（未稅）</th><th>備註</th></tr></thead><tbody>{previewQuote.items.map(item=><tr key={item.QuotationItemId || item.SystemId}><td>{item.SystemCode && item.SystemName ? `${item.SystemCode}－${item.SystemName}` : (item.SystemName || item.SystemCode || '－')}（網路 {item.UserCount} 人版）</td><td className="text-right whitespace-nowrap">NT${Number(item.LineAmount||0).toLocaleString()}</td><td className="text-center">1</td><td className="text-right whitespace-nowrap">NT${Number(item.LineAmount||0).toLocaleString()}</td><td>折數 {item.Discount ?? 100}%<br/>優惠含稅 NT${Number(item.DiscountAmount||0).toLocaleString()}</td></tr>)}</tbody></table>
              <div className="flex justify-end"><table className="w-[52%]"><tbody><tr><td className="w-1/2 text-right">未稅金額</td><td className="text-right">NT${Number(previewQuote.quote.SubtotalAmount||0).toLocaleString()}</td></tr><tr><td className="text-right">含稅金額</td><td className="text-right">NT${Math.round(Number(previewQuote.quote.SubtotalAmount||0)*1.05).toLocaleString()}</td></tr><tr><td className="text-right font-bold">優惠金額（含稅）</td><td className="text-right font-bold">NT${Number(previewQuote.quote.DiscountAmount ?? previewQuote.quote.FinalAmount ?? previewQuote.quote.TotalAmount ?? 0).toLocaleString()}</td></tr></tbody></table></div>
              {previewQuote.isNewPurchase && <section className="mt-3 compact"><h2 className="font-bold text-sm border-b-2 border-black">系統說明</h2>{previewQuote.items.map(item=><div key={`note-${item.SystemId}`} className="mt-1"><b>{item.SystemCode}－{item.SystemName}：</b><span className="whitespace-pre-wrap">{item.Note || '尚未設定系統內容說明'}</span></div>)}<h2 className="font-bold text-sm border-b-2 border-black mt-3">維護說明</h2><ol className="list-decimal pl-5"><li>軟體系統自簽約日起壹年內免費提供教育訓練及維護修復。第二年起各系統年度維護費合計為 NT${previewQuote.maintenanceTotal.toLocaleString()}（含稅）。日後若新增授權人數，各系統每增加一使用者維護費：{previewQuote.items.map((item,i)=><span key={`m-${item.SystemId}`}>{i?'；':''}{item.SystemCode} NT${Number(item.addUserMaintenanceTaxIncluded||0).toLocaleString()}（含稅）</span>)}。</li><li>網路版同時作業人數，各系統每增加一人優惠金額如下（含稅）：{previewQuote.items.map((item,i)=><span key={`l-${item.SystemId}`}>{i?'；':''}{item.SystemCode} NT${Number(item.licenseAddUserTaxIncluded||0).toLocaleString()}</span>)}。</li></ol></section>}
              <table className="mt-3"><tbody><tr className="h-24"><td className="w-[34%] text-center align-top pt-2">客戶確認簽章</td><td className="w-[30%] text-center align-top pt-2"><div className="border-b border-black pb-1 mb-1">報價專用章</div><img src="/seal.JPG" alt="報價專用章" className="h-16 mx-auto object-contain"/></td><td className="w-[36%] p-0"><table className="h-full"><tbody><tr><td className="w-16">承辦人</td><td>產品規劃部副理　鐘廷睿</td></tr><tr><td>電話</td><td>(04)2298-1378#20</td></tr><tr><td>傳真</td><td>(04)2298-1328</td></tr><tr><td>承辦人簽名</td><td><img src="/sign.jpg" alt="承辦人簽名" className="h-7 object-contain"/></td></tr></tbody></table></td></tr></tbody></table>
